...
import { COGNITIVE_EMOTIONAL_PATTERNS_MODULE } from "./config"
import type { CognitiveEmotionalPatternsField } from "./types"
...
export function CognitiveEmotionalPatternsModule() {
  ...
  const getFieldState = (fieldId: string) => {
    const field = COGNITIVE_EMOTIONAL_PATTERNS_MODULE.fields.find((f) => f.id === fieldId)
    return (
      fieldStates[fieldId] || {
  ...
  const renderFieldPreview = (field: CognitiveEmotionalPatternsField) => {
        ...
        return (
          <div className="space-y-3">
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm font-medium text-sage-700">
                      {typeof item === "object" && item.belief
                        ? item.belief
                        : typeof item === "object" && item.trigger
                          ? item.trigger
                          : typeof item === "object" && item.aspect
                            ? item.aspect
                            : "Entry"}
                    </span>
                  </div>
            ...
            <Button
              ...
              onClick={() => {
                let newItem
                if (field.id === "core_beliefs") {
                  newItem = { belief: "", category: "self", strength: "moderate", origin: "" }
                } else if (field.id === "emotional_triggers") {
                  newItem = { trigger: "", emotion: "", intensity: "moderate", context: "" }
                } else if (field.id === "self_concept") {
                  newItem = { aspect: "", content: "" }
                } else if (field.id === "self_esteem_pattern") {
                  newItem = { aspect: "", content: "" }
                } else {
                  newItem =
  ...
  const evolvableCount = COGNITIVE_EMOTIONAL_PATTERNS_MODULE.fields.filter((f) => getFieldState(f.id).evolvable).length
  const requiredCount = COGNITIVE_EMOTIONAL_PATTERNS_MODULE.fields.filter((f) => getFieldState(f.id).required).length
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <Card className="border-sage-200 rounded-organic-xl">
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-2xl font-light text-sage-900">
                {COGNITIVE_EMOTIONAL_PATTERNS_MODULE.name}
              </CardTitle>
              <CardDescription className="text-sage-600 mt-1">
                {COGNITIVE_EMOTIONAL_PATTERNS_MODULE.description}
              </CardDescription>
            </div>
            <Button
              variant="outline"
          ...
          <div className="flex items-center space-x-4 pt-4">
            <Badge variant="outline" className="bg-sage-50">
              {COGNITIVE_EMOTIONAL_PATTERNS_MODULE.fields.length} Fields
            </Badge>
      ...
      <div className="space-y-4">
        {COGNITIVE_EMOTIONAL_PATTERNS_MODULE.fields.map((field, index) => {
...
export default CognitiveEmotionalPatternsModule
