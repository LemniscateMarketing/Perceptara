...
import { WORK_CAREER_MODULE } from "./config"
import type { WorkCareerField } from "./types"
...
export function WorkCareerModule() {
  ...
  const getFieldState = (fieldId: string) => {
    const field = WORK_CAREER_MODULE.fields.find((f) => f.id === fieldId)
    return (
      fieldStates[fieldId] || {
  ...
  const renderFieldPreview = (field: WorkCareerField) => {
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm font-medium text-sage-700">
                      {typeof item === "object" && item.company
                        ? `${item.company} - ${item.position || "Position"}`
                        : "Career Entry"}
                    </span>
                  </div>
                  <Button
  ...
  const evolvableCount = WORK_CAREER_MODULE.fields.filter((f) => getFieldState(f.id).evolvable).length
  const requiredCount = WORK_CAREER_MODULE.fields.filter((f) => getFieldState(f.id).required).length
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <Card className="border-sage-200 rounded-organic-xl">
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-2xl font-light text-sage-900">{WORK_CAREER_MODULE.name}</CardTitle>
              <CardDescription className="text-sage-600 mt-1">{WORK_CAREER_MODULE.description}</CardDescription>
            </div>
            <Button
              variant="outline"
              size="sm"
          ...
          <div className="flex items-center space-x-4 pt-4">
            <Badge variant="outline" className="bg-sage-50">
              {WORK_CAREER_MODULE.fields.length} Fields
            </Badge>
      ...
      <div className="space-y-4">
        {WORK_CAREER_MODULE.fields.map((field, index) => {
...
export default WorkCareerModule
