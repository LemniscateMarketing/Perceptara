...
import { MENTAL_HEALTH_HISTORY_MODULE } from "./config"
import type { MentalHealthHistoryField } from "./types"
...
export function MentalHealthHistoryModule() {
  ...
  const getFieldState = (fieldId: string) => {
    const field = MENTAL_HEALTH_HISTORY_MODULE.fields.find((f) => f.id === fieldId)
    return (
      fieldStates[fieldId] || {
  ...
  const renderFieldPreview = (field: MentalHealthHistoryField) => {
                ...
                <div key={option} className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => {
                      if (checked) {
                        updatePreviewData(field.id, [...selectedValues, option])
                      } else {
                        updatePreviewData(
                          field.id,
        ...
        return (
          <div className="space-y-3">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm font-medium text-sage-700">
                      {typeof item === "object" && item.name
                        ? item.name
                        : typeof item === "object" && item.diagnosis
                          ? item.diagnosis
                          : typeof item === "object" && item.facility
                            ? item.facility
                            : "Entry"}
                    </span>
                  </div>
                  ...
                  {typeof item === "object" ? (
                    Object.entries(item).map(([key, value]) => (
                      <div key={key} className="flex items-center space-x-2">
                        <span className="text-sm font-medium text-sage-700 min-w-[120px] capitalize">
                          {key.replace(/_/g, " ")}:
                        </span>
                        <Input
                          value={String(value)}
            ...
            <Button
              ...
              onClick={() => {
                let newItem
                if (field.id === "current_medications") {
                  newItem = { name: "", dosage: "", prescribing_doctor: "", duration: "", effectiveness: "" }
                } else if (field.id === "past_medications") {
                  newItem = { name: "", dosage: "", duration: "", reason_discontinued: "", side_effects: "" }
                } else if (field.id === "psychiatric_diagnoses") {
                  newItem = { diagnosis: "", diagnosed_by: "", date_diagnosed: "", current_status: "" }
                } else if (field.id === "hospitalization_history") {
                  newItem = { facility: "", dates: "", reason: "", outcome: "" }
                } else if (field.id === "current_mental_health_providers") {
                  newItem = { name: "", type: "", frequency: "", relationship_quality: "" }
                } else {
                  newItem =
        ...
        return (
          <div className="flex items-center space-x-2">
            <Switch
              ...
              onCheckedChange={(checked) => updatePreviewData(field.id, checked)}
            />
            <span className="text-sm text-sage-700">{currentValue ? "Yes" : "No"}</span>
          </div>
        )
  ...
  const evolvableCount = MENTAL_HEALTH_HISTORY_MODULE.fields.filter((f) => getFieldState(f.id).evolvable).length
  const requiredCount = MENTAL_HEALTH_HISTORY_MODULE.fields.filter((f) => getFieldState(f.id).required).length
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <Card className="border-sage-200 rounded-organic-xl">
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-2xl font-light text-sage-900">{MENTAL_HEALTH_HISTORY_MODULE.name}</CardTitle>
              <CardDescription className="text-sage-600 mt-1">
                {MENTAL_HEALTH_HISTORY_MODULE.description}
              </CardDescription>
            </div>
            <Button
              variant="outline"
          ...
          <div className="flex items-center space-x-4 pt-4">
            <Badge variant="outline" className="bg-sage-50">
              {MENTAL_HEALTH_HISTORY_MODULE.fields.length} Fields
            </Badge>
      ...
      <div className="space-y-4">
        {MENTAL_HEALTH_HISTORY_MODULE.fields.map((field, index) => {
                          ...
                          <div className="bg-gradient-to-br from-gray-50 to-slate-50 rounded-organic-xl p-6 border border-gray-200">
                            <div className="flex items-start space-x-3">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="bg-gray-100 rounded-organic-md p-3 border overflow-hidden">
                                  <code className="text-sm font-mono text-gray-800 break-all whitespace-pre-wrap word-break-all">
                                    {field.id}
                                  </code>
                                </div>
                              </div>
...
export default MentalHealthHistoryModule
