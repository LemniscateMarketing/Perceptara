...
import { BEHAVIORAL_PATTERNS_MODULE } from "./config"
import type { BehavioralPatternsField } from "./types"
...
export function BehavioralPatternsModule() {
  ...
  const getFieldState = (fieldId: string) => {
    const field = BEHAVIORAL_PATTERNS_MODULE.fields.find((f) => f.id === fieldId)
    return (
      fieldStates[fieldId] || {
  ...
  const renderFieldPreview = (field: BehavioralPatternsField) => {
        ...
        return (
          <div className="space-y-3">
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm font-medium text-sage-700">
                      {typeof item === "object" && item.behavior
                        ? item.behavior
                        : typeof item === "object" && item.activity
                          ? item.activity
                          : typeof item === "object" && item.mechanism
                            ? item.mechanism
                            : "Entry"}
                    </span>
                  </div>
                      ...
                      <div key={key} className="flex items-center space-x-2">
                        ...
                        {key === "healthy" ? (
                          <div className="flex items-center space-x-2">
                            ...
                            />
                            <span className="text-sm text-sage-700">Healthy mechanism</span>
                          </div>
                        ) : (
                          <Input
                            value={String(value)}
            ...
            <Button
              ...
              onClick={() => {
                let newItem
                if (field.id === "avoidance_behaviors") {
                  newItem = { behavior: "", trigger: "", frequency: "" }
                } else if (field.id === "self_care_routines") {
                  newItem = { activity: "", frequency: "", consistency: "" }
                } else if (field.id === "coping_mechanisms") {
                  newItem = { mechanism: "", effectiveness: "", healthy: false }
                } else if (field.id === "hobbies_interests") {
                  newItem = { activity: "", engagement_level: "", social_aspect: "" }
                } else {
                  newItem =
  ...
  const evolvableCount = BEHAVIORAL_PATTERNS_MODULE.fields.filter((f) => getFieldState(f.id).evolvable).length
  const requiredCount = BEHAVIORAL_PATTERNS_MODULE.fields.filter((f) => getFieldState(f.id).required).length
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <Card className="border-sage-200 rounded-organic-xl">
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-2xl font-light text-sage-900">{BEHAVIORAL_PATTERNS_MODULE.name}</CardTitle>
              <CardDescription className="text-sage-600 mt-1">{BEHAVIORAL_PATTERNS_MODULE.description}</CardDescription>
            </div>
            <Button
              variant="outline"
              size="sm"
          ...
          <div className="flex items-center space-x-4 pt-4">
            <Badge variant="outline" className="bg-sage-50">
              {BEHAVIORAL_PATTERNS_MODULE.fields.length} Fields
            </Badge>
      ...
      <div className="space-y-4">
        {BEHAVIORAL_PATTERNS_MODULE.fields.map((field, index) => {
...
export default BehavioralPatternsModule
